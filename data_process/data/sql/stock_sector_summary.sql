create table `stock_sector_summary`(
    ...
    `total_amount_ratio` decimal(11, 8) not null comment '成交金额-占总计',
    `trade_quantity` decimal(30) not null comment '成交股数-股数',
    `total_quantity_ratio` decimal(11, 8) not null comment '成交股数-占总计',
    `trade_valume` decimal(30) not null comment '成交笔数-笔',
    `total_valume_ratio` decimal(11, 8) not null comment '成交笔数-占总计',
    `year_month` varchar(6) not null comment '日期：yyyymm',
    `type` varchar(8) not null comment '类型：当年|当月'
);